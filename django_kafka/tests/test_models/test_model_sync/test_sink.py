...
class DbzJdbcSinkConfigTestCase(TestCase):
    def _get_config(self, registry=None, model=None, **make_attrs):
        ...
        with (
            mock.patch("django_kafka.conf.settings.CONNECT_DATABASE", _SINK_DB),
            mock.patch(
                "django_kafka.conf.settings.SCHEMA_REGISTRY",
                {"url": "http://sr"},
            ),
        ):
    ...
    def test_collection_name_format(self):
        ...
        self.assertEqual(
            cfg["collection.name.format"],
            f"public.{SimpleModel._meta.db_table}",
        )
        self.assertNotIn("table.name.format", cfg)
