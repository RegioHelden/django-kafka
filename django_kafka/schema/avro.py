...
class AvroSchema:
    ...
    def _get_model_fields(model, fields=None, exclude=False):
        ...
        # exclude=True drops names found in `fields`; exclude=False keeps only
        # those names. The XOR `(name in fields) != exclude` covers both.
        return {
            name: f for name, f in all_fields.items() if (name in fields) != exclude
        }
