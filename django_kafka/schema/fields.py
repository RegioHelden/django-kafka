...
def django_field_to_avro(field):
    ...
    raise ValueError(
        f"Unsupported Django field type: {type(field).__name__} (field: {field.name})",
    )
...
def python_type_to_avro(python_type):
    ...
    # Avro supports unions natively; we only handle T | None → ["null", "str"].
    is_union = isinstance(python_type, types.UnionType) or origin is typing.Union
    if is_union:
